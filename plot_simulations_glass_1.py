...
if __name__ == '__main__':
    ...
    # mc = ps.Simulation_Monte_Carlo('test3_mc', 100_000, radius[0], deltas[0], 0.89, wlum, pol_vectors[1])
    sphere = ps.Simulation_Sphere('test3_sphere_B270', radius[0], deltas[0], 100_000, 100, wlum, pol_vectors[1])
    # ps.plot_Stokes_reflectance_I(mc,fig,ax,label=mc.label,c=c[0])
    ps.plot_Stokes_reflectance_I(sphere,fig,ax,label=sphere.label,c='--'+c[0])
    ax.set_xlim([0,0.04])
    ax.set_ylim([1E-3,2])
    ...
    # mc = ps.Simulation_Monte_Carlo('test3_mc', 100_000, radius[0], deltas[0], 0.89, wlum, pol_vectors[0])
    sphere = ps.Simulation_Sphere('test3_sphere_B270', radius[0], deltas[0], 100_000, 100, wlum, pol_vectors[0])
    ps.plot_DOPL_radius_reflectance(sphere,fig,ax[0,0],label=sphere.label,c=c[0])
    # ps.plot_DOPL_radius_reflectance(mc,fig,ax[0,0],label=mc.label,c='--'+c[0])
    # mc = ps.Simulation_Monte_Carlo('test3_mc', 100_000, radius[0], deltas[0], 0.89, wlum, pol_vectors[1])
    sphere = ps.Simulation_Sphere('test3_sphere_B270', radius[0], deltas[0], 100_000, 100, wlum, pol_vectors[1])
    ps.plot_DOPC_radius_reflectance(sphere,fig,ax[0,1],label=sphere.label,c=c[0])
    # ps.plot_DOPC_radius_reflectance(mc,fig,ax[0,1],label=mc.label,c='--'+c[0])
    ps.plot_MOPL_reflectance(sphere,fig,ax[1,0],label=sphere.label,c=c[0])
    # ps.plot_MOPL_reflectance(mc,fig,ax[1,0],label=mc.label,c='--'+c[0])
    ax[1,0].legend(loc='upper left',prop={'size': 8})
    ps.plot_lair_reflectance(sphere,fig,ax[1,1],label=sphere.label,c=c[0])
    # ps.plot_lair_reflectance(mc,fig,ax[1,1],mc.label,c='--'+c[0])
    ...
    ps.plot_irradiance(sphere,sphere.label,fig,ax,c=['--'+c[0]],TI=True,Tartes=False)
    ...
    # mc = ps.Simulation_Monte_Carlo('test3_mc', 100_000, radius[0], deltas[0], 0.89, wlum, pol_vectors[1])
    sphere = ps.Simulation_Sphere('test3_sphere_B270', radius[0], deltas[0], 100_000, 100, wlum, pol_vectors[1])
    ps.plot_DOPC_transmittance(sphere,fig,ax[0],sphere.label,c=c[0])
    # ps.plot_DOPC_transmittance(mc,fig,ax[0],mc.label,c='--'+c[0])
    # mc = ps.Simulation_Monte_Carlo('test3_mc', 100_000, radius[0], deltas[0], 0.89, wlums[1], pol_vectors[0])
    sphere = ps.Simulation_Sphere('test3_sphere_B270', radius[0], deltas[0], 100_000, 100, wlum, pol_vectors[0])
    ps.plot_DOPL_transmittance(sphere,fig,ax[1],c=c[0],label=sphere.label)
    # ps.plot_DOPL_transmittance(mc,fig,ax[1],c='--'+c[0],label=mc.label)
    ...
    for pol in pol_vectors:
        ...
        sphere = ps.Simulation_Sphere('test3_sphere_B270', radius[0], deltas[0], 100_000, 1000, wlum, pol)
    ...
    for ind_wlum in range(0,len(wlums)):
        ...
        sphere = ps.Simulation_Sphere('test3_sphere_B270', radius[0], deltas[0], 100_000, 100, wlums[ind_wlum], pol_vectors[1])
        # ps.plot_irradiance(mc,mc.label,fig,ax,c=[c[ind_wlum],'--'+c[ind_wlum]],TI=True,Tartes=True)
        ps.plot_irradiance(sphere,sphere.label,fig,ax,TI=True,Tartes=True)
    ...
    # mc = ps.Simulation_Monte_Carlo('test3_mc', 100_000, radius[0], deltas[0], 0.89, wlums[0], pol_vectors[1])
    sphere = ps.Simulation_Sphere('test3_sphere_B270', radius[0], deltas[0], 100_000, 100, wlums[0], pol_vectors[1])
    ps.plot_irradiance(sphere,sphere.label,fig,ax,c=[c[0],'-.'+c[0]],TI=True,Tartes=True,x_axis='musp_theo')
    # ps.plot_irradiance(mc,sphere.label,fig,ax,c=[c[0],'-.'+c[0]],TI=True,Tartes=True,x_axis='musp_theo')
